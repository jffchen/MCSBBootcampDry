...
cell_metadata <- read.table("/Users/jffchen/Desktop/MCSB_R_Bootcamp_2019/cell_metadata.txt")

#extract the normalized data matrix
expression_matrix <- readRDS("/Users/jffchen/Desktop/MCSB_R_Bootcamp_2019/expression_matrix.rds")
```
...
#which selects values from the first vector that match values in the second vector ***which will pull out a index of
...
MKI67_expression <- data.frame(MKI67 = MKI67_df) #convert matrix vector into dataframe

#Step 2: Reorder data frame to match ordering of metadata
MKI67_expression <- MKI67_expression[rownames(cell_metadata),] #caracter indexing, making it back into a vector
...
#0.7382222, so G2M should be dividing and close correlation w/ mki67 shows the g2m score is correct
```
...
```{r q6}
WT_subset <- subset(cell_metadata, orig.ident == "WT") # == means if orig.ident is WT, then keep it for the WT_subset

dim(cell_metadata) #1986 x 9, check dimention of the gene cell matrix
dim(WT_subset) #993 x 9, check dimention of the gene cell matrix
...
#we group our data frame first by the cluster, and next by condition  **will have cluster column, orig ident, and avg ki67, better piviot table**
...
```{r q8}
ggplot(average_mki67_by_cluster_and_condition, mapping = aes(x=res.0.4, y=mean_mki67, fill=orig.ident)) + geom_bar(stat="identity", position="dodge") #position=dodge prevents stacked bargraph
```
...
#Option 2: Use dplyr filter function **dpylr can filter and saves you from making a extra r object
...
matrix_subset_of_interest <- as.matrix(t(genes_of_interest_df)) #t command is transpose. changes gene from row to columns now

#since we have multiple columns to add to the metadata, we will want to 
genes_of_interest_with_necessary_metadata <- merge(cell_metadata[,c("res.0.4","orig.ident")], as.data.frame(matrix_subset_of_interest), by="row.names")  #merge will combine cell metadata and put it into our gene of interest subset

rownames(genes_of_interest_with_necessary_metadata) <- genes_of_interest_with_necessary_metadata$Row.names #r automatically changed rownames
...
#now, we want to reformat this data to generate a list of gene names instead of having each gene with its own column (since we need a single column for the x-axis to be each gene)  **gather is in tydr package., reformat data to work with ggplot
...
# can do scale = "row", normalizes each gene so it shows zscore
```

##Project: Given the following list of cell cycle genes, perform as many tasks as you can in the remaining time. 

```{r cell cycle genes}
cell_cycle_genes <- as.character(read.table("/Users/jffchen/Desktop/MCSB_R_Bootcamp_2019/regev_lab_cell_cycle_genes.txt", header=F)$V1)
```
...
cellcyclegenes_expression_matrix <- as.matrix(expression_matrix)[which(rownames(expression_matrix) %in% cell_cycle_genes),]  # %in% will search for T or F matches between the two indicated objects, "which" will output the row number of the first object in that order.

match(rownames(cellcyclegenes_expression_matrix),cell_cycle_genes) #ensures that the code above worked, will show the corresponding row number of the two inputed objects

dim(cellcyclegenes_expression_matrix)

# add in cell metadata into gene exp list of ccgenes
ccgene <- cellcyclegenes_expression_matrix[rownames(cell_metadata),]
